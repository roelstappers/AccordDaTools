...
# ╔═╡ 58f2dcfe-632d-4246-8cef-04520574ceca
using GLMakie, NCDatasets, Statistics, Dates, FileTrees, PlutoUI

# ╔═╡ 607d2178-c440-4841-92e1-2c3db18ae454
archive="/lustre/storeB/immutable/archive/projects/metproduction/MEPS/"

# ╔═╡ 10575bf9-7ee9-4844-8bc6-d28f0566ce9a
begin 
fcint = Hour(3)
...
get_ands(dtg) = NCDataset(joinpath(archive,constructpath(dtg)))
get_fgds(dtg) = NCDataset(joinpath(archive,constructpath(dtg-fcint)))
ands = get_ands(dtgbeg);
fgds = get_fgds(dtgbeg);
xv = ands["x"][:]
yv = ands["y"][:]
hyb = ands["hybrid"][:]
timev = (ands["time"][1:4] -ands["time"][1])./Hour(1)
nothing
end 


# ╔═╡ 34cd1505-fd9a-4e22-b8d6-f9a424c127ae
md"""
Variable | Value 
:--- | :--- 
dtg    | $dtgbeg
Var    | $(@bind var PlutoUI.Select(vars)) 
X index | $(@bind xi2 PlutoUI.Slider(1:length(xv))) 
Y index | $(@bind yi2 PlutoUI.Slider(1:length(yv))) 
Vert. coord. |  $(@bind vc PlutoUI.Select(["hybrid","Model levels"])) 
"""

# ╔═╡ 9d77db36-0bfa-42c3-84c9-465996bb3dcf
field = ands[var][:,:,:,1:4] - fgds[var][:,:,:,4:7];

# ╔═╡ 10b7d85a-3939-49c5-b5e7-adfa817f1854
begin 
	fig = Figure()
	ax = Axis3(fig[1,1],xlabel="Hybrid", ylabel="time (Hours)",
		title = "$(Dates.format(dtgbeg,"yyyymmddHH")) X=$xi2,  Y= $yi2",
		elevation = pi/2, azimuth=-2pi, protrusions=40
	)	
...
md""" 
Model level $(@bind lev PlutoUI.Slider(1:65) )
"""

# ╔═╡ 955b4b01-b84c-454e-b525-de3131bfd452
begin 

  fig2 = Figure()
  ax3 = Axis(fig2[1,1],title="+00") # , title="$dtgbeg",elevation=pi/2,azimuth=-pi/2)   
  heatmap!(ax3, field[:,:,lev,1],colorrange=crange, colormap=Reverse(:RdBu))
  ax4 = Axis(fig2[1,2],title="+01",yticklabelsvisible=false)# , title="$dtgbeg",elevation=pi/2,azimuth=-pi/2) 
  heatmap!(ax4,field[:,:,lev,2],colorrange=crange, colormap=Reverse(:RdBu))
  ax5 = Axis(fig2[2,1],title="+02") # , title="$dtgbeg",elevation=pi/2,azimuth=-pi/2) 
  heatmap!(ax5,field[:,:,lev,3],colorrange=crange, colormap=Reverse(:RdBu))
  ax6 = Axis(fig2[2,2],title="+03",yticklabelsvisible=false) #, title="$dtgbeg",elevation=pi/2,azimuth=-pi/2) 
  hm =  heatmap!(ax6,field[:,:,lev,4],colorrange=crange, colormap=Reverse(:RdBu))
  Label(fig2[0,:],"Increment $dtgbeg Model level $lev", fontsize=20)

  Colorbar(fig2[1:2,3],hm )
  fig2
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "f6edcfcc7e0c6dacba8ab8c8d43adaf39b59a88a"
...
[[deps.Dagger]]
deps = ["DataStructures", "Distributed", "LinearAlgebra", "MacroTools", "MemPool", "Profile", "Random", "Requires", "ScopedValues", "Serialization", "SharedArrays", "SparseArrays", "Statistics", "StatsBase", "TimespanLogging", "UUIDs"]
git-tree-sha1 = "7346be56f6ad9f2778eb746f3696843cbc9667aa"
