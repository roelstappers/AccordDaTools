...
# ╔═╡ 129ec662-4d70-11ee-001a-1732f99ab4f5
using WGLMakie, Dates, NCDatasets, PlutoUI

# ╔═╡ 05b364b3-7a3d-4767-bf74-ba3300a53858
dtg = Dates.DateTime(2023,04,01,00)

# ╔═╡ 13cffec1-e7d7-48a3-8399-7a069ec04a80
var = "air_temperature_ml"

# ╔═╡ d410cd4c-6e55-4649-9782-e1895da82ac9
@bind ll PlutoUI.Slider(0:1:36)

# ╔═╡ bc6dd6d5-8920-43ed-b46a-74f44e75ff09
@bind lev PlutoUI.Slider(1:65)

# ╔═╡ 6dd5cb61-92eb-4f7c-b468-af6769971473
begin # Helper function for thredds 
  mepsurl="https://thredds.met.no/thredds/dodsC/meps25epsarchive"
  file(dtg) = "meps_det_2_5km_$(Dates.format(dtg,"yyyymmddTHHZ")).nc" 
  url(dtg) = joinpath(mepsurl,Dates.format(dtg,"yyyy/mm/dd"), file(dtg)) 
  querystr(ll,lev,var) = "?$var[$ll][$(lev-1)][0:1:$(nlat-1)][0:1:$(nlon-1)]"
  fcint = Hour(3); nlat=1069; nlon=949; nlev=65  # METCOOP25D
  bg(dtg,lev,var,ll) = NCDataset(url(dtg-fcint) * querystr(ll+fcint.value,lev,var))[var]
  an(dtg,lev,var,ll) = NCDataset(url(dtg) * querystr(ll,lev,var))[var]
  inc(dtg,lev,var,ll) = an(dtg,lev,var,ll)[:,:,1,1] - bg(dtg,lev,var,ll)[:,:,1,1]
  nothing
end 

# ╔═╡ f3399e3e-d0d6-484a-9b7d-3a6bc9888f91
md"""
# Metcoop DA increments
...
# ╔═╡ 164d9a53-92f4-4314-b18c-d50b3bd8fc19
begin 
	fig = Figure(resolution=(1200,1200))
	ax = Axis(fig)
	heatmap(inc(dtg,lev,var,ll))
	# xlabel!(plt,"gi")
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "a8794b047a84784dae607f689ba8b6f5181884ce"
